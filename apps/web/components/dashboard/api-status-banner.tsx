...
export function ApiStatusBanner({ error, isLoading, onRetry }: ApiStatusBannerProps) {
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 leading-6">
              CrocLens could not reach the local API at <span className="font-semibold">{getApiBaseUrl()}</span>. {error}
            </p>
          </div>
        </div>
        <button
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-emerald-900/10 bg-white/80 px-4 py-3 text-sm text-stone-700 shadow-card">
      ...
      Dashboard data is connected to <span className="font-semibold text-croc-moss">{getApiBaseUrl()}</span>
    </div>
  );
}
